...
export const setupSadFace = async () => {
    return new Promise(function (resolve, reject) {
        if (!LIS.id('github')) {
            return resolve('### function "setupSadFace" ignored well')
        }
        try {
            setTimeout(() => {
                scaleSmile(0)
            ...
            return resolve('### function "setupSadFace" run successfully')
        } catch (error) {
            console.log(error.message)
            return reject(new Error('### function "setupSadFace" failed'))
        }
    })
}
